...
//     date: "2023-08-05",
//     excerpt:
//       "Aquí tienes nuestro segundo artículo del blog con contenido interesante.",
//   },
//   // Agrega más artículos del blog según sea necesario
// ];

function Blog() {
  const { data } = api.blog.getRecentPosts.useQuery();

  return (
    <PageLayout pageTitle="Blog">
      <BsCenteredContainer extraClasses="mt-8">
        <h1 className="block text-5xl font-semibold uppercase">
          Últimos artículos
        </h1>
        <div className="">
          {data?.map((post) => (
            ...
            //   <p>{post.excerpt}</p>
            // </li>
            <div key={post.id} className="mt-12 flex flex-col">
              ...
              {/* {data?.map((pic) => (
                <p key={pic.id}>{pic.profile_pic_url}</p>
              ))} */}
            </div>
          ))}
        </div>
